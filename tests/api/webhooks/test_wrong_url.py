...
async def test_wrong_url(client: AsyncClient, webhook_wrong_url: WebhookEndpoint, uow: SQLAlchemyUOW) -> None:
    response = await client.post("/register", json=USER_CREATE.model_dump(mode="json"))
    assert response.status_code == status.HTTP_201_CREATED

    try:
        delivery = await uow.webhook_deliveries.find(WebhookDeliveryEndpointIDSpecification(webhook_wrong_url.id))
